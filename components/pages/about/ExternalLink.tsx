...
const ExternalLink = ({ name, icon: Icon, link }: ExternalLinkProps) => {
    return (
        <Link href={link} className="group/link flex items-center gap-4" aria-label={name}>
            <Icon
                className="text-primary-900 w-12 aspect-square transition-colors group-hover/link:text-primary-700"
            />
            <span className="text-xl font-medium underline transition-colors group-hover/link:text-neutral-400">
                {name}
            </span>
        </Link>
    );
